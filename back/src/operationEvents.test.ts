...
test("detecta una subida de presupuesto y la reporta en pesos", () => {
  ...
  const [ev] = detectarCambios({
    ...
    antes: { dailyBudgetRaw: "1400000" },
    despues: { dailyBudgetRaw: "2000000" },
  });
...
test("una bajada se reporta como bajada", () => {
  const [ev] = detectarCambios({
    ...base,
    antes: { dailyBudgetRaw: "3100000" },
    despues: { dailyBudgetRaw: "1800000" },
  });
...
test("LA INVARIANTE: la primera vez no inventa cientos de cambios", () => {
  ...
  assert.deepEqual(
    detectarCambios({ ...base, antes: {}, despues: { dailyBudgetRaw: "3100000" } }),
    [],
  );
  assert.deepEqual(
    detectarCambios({ ...base, antes: { dailyBudgetRaw: null }, despues: { dailyBudgetRaw: "3100000" } }),
    [],
  );
  // Y al revés: si Meta deja de devolver el campo, tampoco es un cambio de operación.
  assert.deepEqual(
    detectarCambios({ ...base, antes: { dailyBudgetRaw: "3100000" }, despues: {} }),
    [],
  );
});
...
test("sin cambios no emite nada", () => {
  assert.deepEqual(
    detectarCambios({
      ...base,
      antes: { dailyBudgetRaw: "3100000", bidStrategy: "LOWEST_COST_WITHOUT_CAP", effectiveStatus: "ACTIVE" },
      despues: { dailyBudgetRaw: "3100000", bidStrategy: "LOWEST_COST_WITHOUT_CAP", effectiveStatus: "ACTIVE" },
    }),
    [],
  );
});
...
test("varios cambios a la vez salen como eventos separados", () => {
  const evs = detectarCambios({
    ...base,
    antes: { dailyBudgetRaw: "2900000", bidStrategy: "COST_CAP", effectiveStatus: "ACTIVE" },
    despues: { dailyBudgetRaw: "3500000", bidStrategy: "LOWEST_COST_WITHOUT_CAP", effectiveStatus: "ACTIVE" },
  });
