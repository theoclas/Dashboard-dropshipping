...
export type UnifiedImportScopeInput =
  | {
      kind: "product";
      catalogProductId: string;
  ...
  | { kind: "all"; advertisingAccountIds?: string[]; selectedCampaignIds?: string[] };

export type UnifiedResolvedScope = {
  kind: "product" | "all";
...
export type UnifiedUnlinkedCampaign = {
  campaignId: string;
  externalCampaignId: string;
  displayName: string | null;
...
export type UnifiedLinkConflict = {
  campaignId: string;
  externalCampaignId: string;
  displayName: string | null;
...
export type UnifiedValidateRangeResponse = {
  ok: boolean;
  message?: string;
  desde: string;
  hasta: string;
  days: number;
  maxDays: number;
  chunks: UnifiedRangeChunk[];
  chunkDays: number;
};

export type UnifiedPreviewResponse = {
  runId: string;
...
export type UnifiedDryRunRow = {
  externalCampaignId: string;
  campaignName: string | null;
  ymd: string;
  spend: number;
  /** Gasto que hay hoy en base; `null` si la fila aún no existe. */
...
export type UnifiedDryRunResponse = {
  runId: string;
  scope: UnifiedResolvedScope;
  desde: string;
  hasta: string;
